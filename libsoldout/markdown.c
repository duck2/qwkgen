...
/* char_mathspan • '~' parsing inline math */
static size_t
char_mathspan(struct buf *ob, struct render *rndr,
				char *data, size_t offset, size_t size) {
	size_t end, nb = 0, i, f_begin, f_end;

	/* counting the number of ~s in the delimiter */
	while (nb < size && data[nb] == '~') nb += 1;
	...
	if (f_begin < f_end) {
		struct buf work = { data + f_begin, f_end - f_begin, 0, 0, 0 };
		if (!rndr->make.mathspan(ob, &work, rndr->make.opaque))
			end = 0; }
	else {
		if (!rndr->make.mathspan(ob, 0, rndr->make.opaque))
			end = 0; }
	return end; }
...
/* prefix_codefence • returns prefix length for code fence ``` */
static size_t
prefix_codefence(char *data, size_t size) {
	if (size < 4) return 0;
...
/* prefix_blockmath • returns prefix length for block math ~~~ */
static size_t
prefix_blockmath(char *data, size_t size) {
	if (size < 4) return 0;
	if (data[0] == '~' && data[1] == '~' &&
		data[2] == '~' && data[3] == '\n') return 4;
	else return 0; }
...
/* parse_codefence • handles parsing of a block-level code fence ``` */
static size_t
parse_codefence(struct buf *ob, struct render *rndr,
			char *data, size_t size) {
	size_t beg = 0, end;
	struct buf *work = new_work_buffer(rndr);

	/* jump over "```javascript\n" */
	beg += prefix_codefence(data, size);

	/* scan lines and copy until the "```\n" line */
	while (beg < size) {
		for (end = beg + 1; end < size && data[end - 1] != '\n';
							end += 1);
		if (data[beg] == '`' && data[beg+1] == '`' &&
			data[beg+2] == '`' && data[beg+3] == '\n'){
			beg += 4;
			break;
		}
		if (beg < end) {
...
/* parse_blockmath • handles parsing of block-level math */
static size_t
parse_blockmath(struct buf *ob, struct render *rndr,
			char *data, size_t size) {
	size_t beg = 0, end;
	struct buf *work = new_work_buffer(rndr);

	/* jump over "~~~\n" */
	beg += prefix_blockmath(data, size);

	/* scan lines and copy until the "~~~\n" line */
	while (beg < size) {
		for (end = beg + 1; end < size && data[end - 1] != '\n';
							end += 1);
		if (prefix_blockmath(data+beg, size-beg)){
			beg += 4;
			break;
		}
		if (beg < end) {
	...
	if (rndr->make.blockmath)
		rndr->make.blockmath(ob, work, rndr->make.opaque);
	release_work_buffer(rndr, work);
	return beg; }
...
parse_block(struct buf *ob, struct render *rndr,
	...
	while (beg < size) {
		...
		else if (rndr->make.blockmath && prefix_blockmath(txt_data, end))
			beg += parse_blockmath(ob, rndr, txt_data, end);
		else if (prefix_codefence(txt_data, end))
			beg += parse_codefence(ob, rndr, txt_data, end);
...
markdown(struct buf *ob, struct buf *ib, const struct mkd_renderer *rndrer) {
	...
	if (rndr.make.mathspan) rndr.active_char['~'] = char_mathspan;

...
/* duck2's math extensions */

/* write the base64-encoded png data for the
 * incoming latex expression to the output buffer
 * return 1 on success, 0 on failure. */
static int
latex2png(struct buf *ob, struct buf *latex){
	if (!latex || !latex->size) return 0;

	char pipebuf[4096];
	struct buf * cmd = bufnew(64 + latex->size);
	latex->data[latex->size] = '\0';
	bufprintf(cmd, "bin/latex2png '%s'", latex->data);
	bufnullterm(cmd);

	FILE *fp = popen(cmd->data, "r");
	if(fp == NULL) return 0;
	while(fgets(pipebuf, 4096, fp) != NULL)
		bufputs(ob, pipebuf);
...
static void
d2_blockmath(struct buf *ob, struct buf *text, void *opaque){
	struct buf * png = bufnew(4096);

	if(!latex2png(png, text)){
		BUFPUTSL(ob, "<p>");
		bufput(ob, text->data, text->size);
		BUFPUTSL(ob, "</p>\n");
		return;
	...
	BUFPUTSL(ob, "<img class=\"block-math\" src=\"data:image/png;base64,");
	bufput(ob, png->data, png->size);
	BUFPUTSL(ob, "\" title=\"");
	bufput(ob, text->data, text->size);
	BUFPUTSL(ob, "\"/>\n");
}
...
static int
d2_mathspan(struct buf *ob, struct buf *text, void *opaque){
	if (!text || !text->size) return 0;
	struct buf * png = bufnew(4096);

	if(!latex2png(png, text)) return 0;

	BUFPUTSL(ob, "<img class=\"inline-math\" src=\"data:image/png;base64,");
	bufput(ob, png->data, png->size);
	BUFPUTSL(ob, "\" title=\"");
	bufput(ob, text->data, text->size);
	BUFPUTSL(ob, "\"/>\n");
...
const struct mkd_renderer d2_html = {
	NULL,
	NULL,
